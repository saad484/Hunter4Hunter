...
Hello! My name is saad ([jktrn](https://github.com/saad484) on GitHub), and I'm a fullstack web developer who has been fiddling with blogging platforms for a couple of years now. I run a blog at [hunter4hunter.dev](https://hunter4hunter.dev), where I write about cybersecurity and the capture-the-flag (CTF) scene.
...
- You can specify multiple post authors via frontmatter. If this post author's id is found within the `Authors` collection, then it will render particular info from that author's frontmatter file, `[author-name].md` (e.g. avatar, link to profile). For example, the previous post (2024 Post) has two authors: "saad484".
